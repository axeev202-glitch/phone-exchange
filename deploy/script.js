...
// Проверяем, есть ли префикс /www в текущем пути
// Если да, добавляем его к API путям
const currentPath = window.location.pathname;
const hasWwwPrefix = currentPath.startsWith('/www') || currentPath.startsWith('/www/');

// Определяем базовый URL для API
let apiBase;
if (apiBaseOverride && apiBaseOverride.length > 0) {
    apiBase = apiBaseOverride;
} else if (isLocalhost) {
    apiBase = 'http://localhost:3000';
} else {
    // Используем origin (без пути), так как API всегда на корне домена
    apiBase = window.location.origin;
}

// Добавляем префикс /www к API путям, если страница находится в /www
const apiPrefix = hasWwwPrefix ? '/www' : '';

const API_URL = `${apiBase.replace(/\/$/, '')}${apiPrefix}/api/listings`;
const USERS_API_URL = `${apiBase.replace(/\/$/, '')}${apiPrefix}/api/users`;
// Username бота для открытия мини‑аппа по ссылке профиля
const BOT_USERNAME = 'ObmenTech_bot';

console.log('🌐 API Base URL:', apiBase);
console.log('🔍 Current pathname:', currentPath);
console.log('🔍 Has /www prefix:', hasWwwPrefix);
console.log('🔍 API prefix:', apiPrefix);
console.log('📡 API URL (listings):', API_URL);
console.log('👤 API URL (users):', USERS_API_URL);
